    ...
    <script !src="">
        ...
        $(document).ready(function(){
            ...
            var table = $("#storeTable").DataTable({
                ajax: "{{url('manager/purchase/show')}}",
                "pageLength": 50,
                ordering: false,
                columns: [
                    {data: "id"},
            ...
            $("#supplierID").select2({
                ...
                ajax: {
                    url:'{{url('manager/purchase/supplier')}}',
            ...
            $("#productID").select2({
                ...
                ajax: {
                    url:'{{url('manager/purchase/product')}}',
                    processResults: function (data) {
                        var data = $.parseJSON(data);
                        return {
                            results: data
            ...
            $(document).on("click", ".btn-edit", function () {
                var id = $(this).val();
                $.ajax({
                    url: "{{url('manager/purchase')}}/" + id + "/edit",
                    headers: {
                        'X-CSRF-TOKEN': token
                    },
            ...
            $(document).on("click", "#submit", function () {
                ...
                if (type === 'Update') {
                    $.ajax({
                        type: "PUT",
                        url: "{{url('manager/purchase')}}/" + purchaseID,
                        data: {
                            'date': date.val(),
                            'invoiceID': invoiceID.val(),
                            'productID': productID.val(),
            ...
            $(document).on('click', '.btn-status', function () {
                ...
                $.ajax({
                    type: "post",
                    url: "{{url('manager/purchase/status')}}",
                    data: {
                        'status': status,
                        'id': id,
                        '_token': token
