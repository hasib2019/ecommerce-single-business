        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <div class="row mb-2">
                        <div class="col-md-6">
                            <h4 class="page-title mt-0 d-inline">Total <span class="total">0</span> Supplier </h4>
                        </div>
                        <div class="col-md-6">
                            <div class="text-md-right">
                                <button type="button" class="btn btn-blue btn-add btn-xs waves-effect waves-light float-right"><i class="mdi mdi-plus-circle mr-1"></i> Add New Supplier</button>
                            </div>
                        </div>
                    </div>
    ...
    <script !src="">
        ...
        $(document).ready(function(){
            var table = $("#storeTable").DataTable({
                ajax: "{{url('manager/supplier/show')}}",
                "pageLength": 50,
                ordering: false,
                columns: [
                    {data: "id"},
            ...
            $(document).on("click", ".btn-edit", function () {
                var id = $(this).val();
                $.ajax({
                    url: "{{url('manager/supplier/')}}/" + id + "/edit",
                    headers: {
                        'X-CSRF-TOKEN': token
                    },
                    ...
                    success: function (response) {
                        ...
                        $('.modal .modal-title').empty().text('Edit Supplier');
            ...
            $(document).on("click", "#submit", function () {
                ...
                if (type === 'Update') {
                    $.ajax({
                        type: "PUT",
                        url: "{{url('manager/supplier')}}/" + supplierID,
                        data: {
                            'supplierName': supplierName.val(),
                            'supplierPhone': supplierPhone.val(),
                            '_token': token
            ...
            $(document).on('click', '.btn-status', function () {
                ...
                $.ajax({
                    type: "post",
                    url: "{{url('manager/supplier/status')}}",
                    data: {
                        'status': status,
                        'id': id,
                        '_token': token
